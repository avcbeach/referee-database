...
def page_referee_search():
    ...
    # ====================================
    # 🗑️ DELETE REFEREE (PROFILE PAGE)
    # ====================================
    st.markdown("---")
    st.subheader("🗑️ Delete Referee")

    ref_name = f"{prof['first_name']} {prof['last_name']} ({prof['nationality']})"
    ...
    confirm_del = st.checkbox("Yes, I understand. Delete permanently.", key="del_confirm_profile")

    if st.button("🗑️ Delete Referee", key="del_ref_profile"):
        if confirm_del:
            refs_all = load_referees()
